...
1) Install Python 3.14.2 from Microsoft Store.
...
3) Create a text file with list of tickers for which to retrieve quotes for. Format of the text file is one 
   ticker per line. While each blank line is ignored, any non-blank line is assumned to be a ticker symbol. An
...
4) Format of invocation is as follows with optional fixed positional parameters:
    ...
    Example (to retrieve today's prices):
        py -V:3.14.2  get_historic_quotes.py my_tickers 1

    Of course if you don't specify parametes then the script uses few hard coded tickers and retrieves 30 days
...
While I may not be able to respond to all comments, suggetions and criticisiom personally and in timely manner,
